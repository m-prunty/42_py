...
class Start:
    ...
    def render_start(self) -> None:
        ...
        size = self.cfg.window_siz
        start_btn = Textures.load(
            "start_button.png",
            Vec2(size.x / 2, size.y / 7),
            (0,),
        )[0]
        ...
        opt_logo = Textures.load(
            "options_button.png",
            Vec2(size.x / 10, size.x / 10),
            (0,),
        )[0]
        Renderer.render_text("A-MAZE-ING", Vec2((size.x / 2) - 50, 50))
        Renderer.render_image(start_btn, Vec2(size.x / 4, size.y / 7))
        Renderer.render_image(filemap_btn, Vec2(size.x / 4, size.y * 2 / 7))
        Renderer.render_image(
            opt_logo, Vec2(size.x * 8.5 / 10, size.y * 8.5 / 10)
        )
        self.add_hooks()
        Event_loop.launch()
